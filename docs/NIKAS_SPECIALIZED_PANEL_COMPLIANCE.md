...
| Vehicle silhouette proportions and anchors | PASS / FIELD CHECK | Both vehicles retain the common 72% width, centre and wheel line. The 683-й alpha frame remains the source reference; a visible 1.04 landmark-height correction compensates the 130-й artwork's smaller wheel/body geometry in every state. |
| Operational values remain readable | PASS / FIELD CHECK | The 74 px row is allocated 40/38/22 percent to engine/event/parking after measuring the full approved-size strings. Every icon stays in a fixed left column, labels do not wrap, and engine/parking values remain single-line without lowering typography floors. |
...
4. Confirm all state-scene variants on both cars: common width, horizontal centre and wheel line must remain stable; the 130-й landmark correction must be visibly taller than v0.6.4 without transition jumps.
