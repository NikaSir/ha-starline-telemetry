...
Audit target: UI/integration 0.6.4 against NikaS specialized-panel standard v1.9 and Navigation Contract v1.1. Static bundle checks pass; final iPhone field acceptance remains required.
...
| Vehicle silhouette proportions and anchors | PASS / FIELD CHECK | Both vehicles retain the common 72% visible width, centre and wheel line; every 130-й state is rendered with the literal visible proportions of the corresponding 683-й reference state. |
| Operational values remain readable | PASS / FIELD CHECK | The engine receives one third of the existing 74 px row. Every icon stays in a fixed left column while its label and value share one aligned text column; labels do not wrap and typography floors are unchanged. |
...
4. Confirm all state-scene variants on both cars: their visible alpha frames must retain the common 72% width, horizontal centre and wheel line; the 130-й state must use the corresponding 683-й visible proportions without flattening.
