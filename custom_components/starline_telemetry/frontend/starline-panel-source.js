...
class StarLineAppPanel extends HTMLElement {
  ...
  _scene(vehicle) {
    ...
    return { id, state, geometry: CAR_ASSET_GEOMETRY[id][state], src: `${ASSET_BASE}/starline-car-${id}-${suffix}.webp?v=${UI_VERSION}` };
  }

  _applyCarGeometry(frame, geometry) {
    if (!frame || !geometry) return;
    const [canvasWidth, _canvasHeight, alphaLeft, alphaTop, alphaRight, alphaBottom] = geometry;
    ...
    frame.style.setProperty("--car-visible-aspect", `${visibleWidth} / ${visibleHeight}`);
  ...
  _mountSummary(pane, vehicle) {
    ...
    this._applyCarGeometry(this.$(".summary-car-frame", pane), scene.geometry);
  }
