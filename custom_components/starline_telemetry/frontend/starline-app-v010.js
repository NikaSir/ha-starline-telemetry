...
class StarLineAppPanelV010 extends BASE_COMPONENT {
  ...
  _installZoomLayer() {
    ...
    content.addEventListener("touchmove", (event) => {
      ...
      const next = this._pinch.scale * (currentDistance / this._pinch.distance);
      this._setScale(next, midpoint(event.touches));
    }, { passive: false });
  ...
  _installZoomStyles() {
    ...
    style.textContent = `
      ...
      @media (max-width:699px) {
        .zoom-controls { bottom:8px; margin-top:12px; }
      }
    `;
